...
public class DataClass {
	...
	public static void main(String[] args) throws ParseException {
		...
		System.out.println("年份：" + date.getYear());// 代表方法过时了。

		Calendar calendar = Calendar.getInstance(); // 获取当前的系统时间。//注：这个类是protected的，本应extends。然而直接获取instance就可以
		System.out.println("年：" + calendar.get(Calendar.YEAR));
		System.out.println("月：" + (calendar.get(Calendar.MONTH) + 1));// 国外月份从0开始算
		...
		// * 日期格式化类 SimpleDateFormat     重要   还有Date
		// * 作用1： 可以把日期转换转指定格式的字符串 format()
		// * 作用2： 可以把一个 字符转换成对应的日期。 parse() 生日
		// *

		SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy年MM月dd日   HH:mm:ss"); // 使用了默认的格式创建了一个日期格式化对象。（）即是默认。（"写上格式"）写什么格式都行
