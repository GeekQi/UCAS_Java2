...
//		}
		...
		while (it.hasNext()) {
			...
			System.out.println("元素：" + temp);
			 it.remove();//remove需要迭代器的对象才能调用。for循环不能
		}
		...
		// 需求： 在控制台打印5句hello world.。。。可知：普通for循环能做的。增强for循环需要遍历的目标：至少得5个元素：这样就太烦了。
		...
		// 注意：
		// Map集合没有实现Iterable接口，所以map集合不能直接使用增强for循环，如果需要使用增强for循环需要借助于Collection
		// 的集合。也就是单例集合
