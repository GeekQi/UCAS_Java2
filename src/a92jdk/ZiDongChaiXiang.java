...
public class ZiDongChaiXiang {
	...
	public static void main(String[] args) {
		...
		//可以把字符串当成对应的进行数据帮你转换
		String data = "10";
		int a = Integer.parseInt(data, 2);//查看源代码：最大能为36
		System.out.println("a="+a);
		
		
		//集合： 集合是可以存储任意对象类型数据的容器。
		ArrayList list = new ArrayList();
		list.add(1);//这是基本类型的啊。正是加入了自动装箱。
		list.add(2);
		list.add(3);
		
		//自动装箱： 自动把java的基本数据类型数据转换成对象类型数据。
		int temp = 10;  //基本数据类型
		Integer b =temp; //把a存储的值赋予给b变量。//Integer是个类。所以是引用数据类型
//		Integer b =new Integer(temp);//这个步骤就是自动装箱。简化了这个步骤
...
//		Integer e = 128;
//		Integer f = 128; 
		Integer e = 120;//这是同一个对象。但是128就不是同一个对象
		Integer f = 120; 
		System.out.println("同一个对象吗？"+(e==f)); // Integer类内部维护 了缓冲数组，该缓冲数组存储的-128~127 这些数据在一个数组中。节省内存空间
