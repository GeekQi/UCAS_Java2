...
/*
 线程常用的方法：
	 ...
	 sleep()                 线程睡眠指定的毫秒数。 静态的方法（不需要对象调用）， 那个线程执行了sleep方法代码那么就是那个线程睡眠。注：如果在main中，所有代码都是由主线程执行的
	 //直接Thread.sleep()都是可以的
	  * 
...
public class ThreadMethod extends Thread {
	...
	public void run() {
		...
		// this 和当前对象是同一个。是该对应函数调用者对象。

		for (int i = 0; i < 10; i++) {
			System.out.println(Thread.currentThread().getName() + ":" + i);
			// try {
			//// Thread.sleep(10);// 为什么在这里不能抛出异常，只能捕获？？
	...
	public static void main(String[] args) throws InterruptedException {
		...
		ThreadMethod d = new ThreadMethod("狗娃");
		System.out.println("线程的名字" + d1.getName());

		d.setName("铁蛋"); // setName设置线程的名字

		d.setPriority(10); // 设置线程 的优先级。 优先级的数字越大，优先级越高 ，
							// 优先级的范围是1~10//但也没说一定要先执行完。
		d.start(); // 开启线程
